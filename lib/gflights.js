...
// Exports searchOne(browser, origin, dest, date, ret) -> { ..., rows:[normalized] }
// CLI single run: node scripts/gflights.js "Paris" "Miami" 2026-08-13 [2026-08-20]
//   (4th arg optional = return date -> round-trip)
...
function parseAria(al) {
  ...
  // Round-trip labels read "From 276 euros round trip total ..." and describe only the
  // OUTBOUND leg (times/stops/duration below). The total price is the full round trip;
  // the leg details are outbound-only, so tag them as such (Q3 decision).
  const roundTrip = /round\s*trip\s*total/i.test(al);
  return {
    ...
    legDetails: roundTrip ? 'outbound-only' : 'full',
  };
}

// ret (optional): a return date (yyyy-mm-dd). When present, query Google's own
// bundled ROUND-TRIP option ("... on <dep> through <ret>"); when absent, one-way.
async function searchOne(browser, origin, dest, date, ret) {
  const q = ret
    ? `Flights to ${dest} from ${origin} on ${date} through ${ret}`
    : `Flights to ${dest} from ${origin} on ${date} one way`;
  const url = `https://www.google.com/travel/flights?q=${encodeURIComponent(q)}&curr=EUR&hl=en&gl=US`;
  ...
  try {
    ...
    rows = labels.map(parseAria).filter(Boolean).map((r) => ({ source: 'google_flights', origin, dest, date, returnDate: ret || null, ...r }));
  } catch (e) { error = e.message.split('\n')[0]; }
  await ctx.close();
  return { origin, date, returnDate: ret || null, challenge, error, count: rows.length, rows };
}
...
if (require.main === module) {
  (async () => {
    const [o = 'Paris', d = 'Miami', date = '2026-08-13', ret] = process.argv.slice(2);
    const b = await chromium.launch({ headless: true });
    console.log(JSON.stringify(await searchOne(b, o, d, date, ret), null, 2));
    await b.close();
  })();
}
